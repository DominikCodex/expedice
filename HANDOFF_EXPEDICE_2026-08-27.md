...
Upřesnění 18. 9. 2026: hlavní požadavek uživatele je samostatné tlačítko v Excelu pro tiskovou sestavu A4 na šířku, bez přihlášení a bez tokenu. Modul `vba/VyskladneniTisk.bas` se instaluje samostatně podle `vba/VyskladneniTisk.md`; žádné existující makro se neslučuje ani nemění. `POST /api/warehouse/render-print` vrací hotové HTML s vloženými daty, CSS a JS. Excel odpověď uloží do `%TEMP%\ExpediceVyskladneni` a otevře lokálně. Tato cesta nezapisuje dávky a neumožňuje přístup k uloženým expedičním datům; vrací jen zaslané řádky a veřejné URL fotografií k zaslaným kódům. Limit 2 MB / 1000 řádků. Starší chráněný `/api/warehouse/upload-print` a přihlášené prohlížení `warehouse-print.html?dataset=<id>` zůstávají funkční, ale nové makro je nepoužívá. Stránka používá původní množství, produktové fotografie, volitelné řazení, opakovanou tiskovou hlavičku a ruční odškrtávací políčka. Serverové testy jsou v `tests/test_warehouse_print.py`, browser/PDF kontroly v `tests/e2e/warehouse-print.spec.js`, včetně skutečného HTML vygenerovaného serverem a otevřeného přes `file://` bez API volání. Starší již nainstalovaný VBA modul je nutné aktualizovat v sešitu, název makra se nemění.
